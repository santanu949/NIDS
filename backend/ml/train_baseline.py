...
TRAIN_CSV = PROJECT_ROOT / "backend" / "data" / "raw" / "UNSW_NB15_training-set.csv"
SCHEMA_PATH = PROJECT_ROOT / "backend" / "data" / "feature_schema.json"
PREPROCESSOR_PATH = PROJECT_ROOT / "backend" / "ml" / "artifacts" / "preprocessor.joblib"

ARTIFACT_DIR = PROJECT_ROOT / "backend" / "ml" / "artifacts"
MODEL_PATH = ARTIFACT_DIR / "logistic_regression.joblib"
RESULT_PATH = ARTIFACT_DIR / "logistic_regression_results.json"
...
def main() -> None:
    print("=" * 65)
    print("  UNSW-NB15 Phase 3 – Logistic Regression Baseline")
    print("=" * 65)
    ...
    print(f"  Preprocessor scaler samples: {scaler.n_samples_seen_}")
    ...
    print(f"  X_val transformed:   {X_val_transformed.shape}")
    ...
    print("                 Normal  Attack")
    print(
        f"Actual Normal    {metrics['true_negative']:6d}  "
        f"{metrics['false_positive']:6d}"
    )
    print(
        f"Actual Attack    {metrics['false_negative']:6d}  "
    ...
    results = {
        ...
        "class_distribution": {
            ...
            "validation": {
                str(k): int(v)
                for k, v in y_val.value_counts().sort_index().items()
            },
        },
        "preprocessing": {
